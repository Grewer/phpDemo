	...
	Vue.component('my-list', {
	  ...
	  methods:{
	  		...
	  		share:function(m,e){
	  			...
	  			var tmp = Vue.extend({ 
		              template: '<div  class="goog-menu goog-menu-vertical share-menu" style="user-select: none; visibility: visible; left: 208.891px; top: -216px;"><div class="goog-menuitem" role="menuitem" id=":1l" style="user-select: none;"><div  @click="weibo" class="goog-menuitem-content" style="user-select: none;"><span class="ico z-ico-weibo-share" style="user-select: none;"></span>新浪微博</div></div><div class="goog-menuitem goog-menuitem-disabled" role="menuitem" aria-disabled="true" id=":1m" style="user-select: none;"><div class="goog-menuitem-content" style="user-select: none;" ref="wechatshare" ><span class="ico z-ico-wechat-share"  style="user-select: none;"></span>微信扫一扫</div></div><div class="goog-menuitem" role="menuitem" id=":1n" style="user-select: none;"><div class="goog-menuitem-content" style="user-select: none;" @click="qq"><span class="ico z-ico-daily-share" style="user-select: none;"></span>QQ空间</div></div></div>',
		              methods:{
		                  weibo:function(){
		                  	var obj = $(e.target).closest('.feed-item');
		                  	window.sharetitle = obj.find('.question_link').text();
		                  	weiboshare();
		                  },
		                  qq:function(){
		                  	var content = $(e.target).closest('.feed-item').find('.question_link').text();
		                  	qqshare(content);
		                  }
		              },
		              mounted:function(){
		              	var obj = $(e.target).closest('.feed-item');
	                    var content = this.$refs.wechatshare;
	                    var qrcode = new QRCode(content,{
	                    	width: 94,
                        	height: 94
	                    });
	                    var defaultContent = obj.find('.question_link').text();
